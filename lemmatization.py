...
def lemmifier(text: str):
    nlp = spacy.load("en_core_web_sm", disable=['parser', 'ner'])
    doc = nlp(text)
    return " ".join([word.lemma_ for word in doc])


def file_pointer(ver) -> int:
    if os.path.exists(f'{year}{ver}.csv'):
        return int(subprocess.check_output(
            ['wc', '-l', f'{year}{ver}.csv']).decode("utf-8").split(' ')[0])
    else:
        return 0
    ...
    for year in years:
        ...
        while row < df.shape[0]:
            with ProcessPoolExecutor() as exe:
                article = df.iloc[row]['article']
                article = exe.map(lemmifier(article), [
                    i for i in range(row, row + chunk)])

            df.iloc[row:row + chunk].to_csv(
                f"{year}_02.csv", mode='a', header=not os.path.exists(
                    f'{year}_02.csv'), index=False)
