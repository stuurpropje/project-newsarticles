...
def df_apply(t: Time, df: pd.DataFrame, column: str, function: str) -> None:
    tqdm.pandas()
...
def remove_stopwords(article: str) -> str:
    ...
    return (' '.join([word for word in article.split() if word not in filler]))
...
if __name__ == "__main__":
    ...
    for year in years:
        ...
        df_apply(
            t, df, 'title', lambda x: x.translate(str.maketrans('', '', ','))
            if type(x) == str else x)

        print('Removing commas from author names...')
        df_apply(
            t, df, 'author', lambda x: x.translate(str.maketrans('', '', ','))
            if type(x) == str else x)
